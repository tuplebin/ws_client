...
start_link(URL, Handler, HandlerArgs, Opts) when is_list(Opts) ->
    ...
    #{scheme := ProtoStr, host := Host, path := Path} = UriMap,
    ...
    proc_lib:start_link(?MODULE, ws_client_init,
        [Handler, Protocol, Host, Port, Api, HandlerArgs, Opts]).


%% Send a frame asynchronously
-spec cast(Client :: pid(), Frame :: ws_req:frame()) ->
                  ok.
cast(Client, Frame) ->
    Client ! {cast, Frame},
    ok.
...
-spec ws_client_init(Handler :: module(), Protocol :: ws_req:protocol(),
...
ws_client_init(Handler, Protocol, Host, Port, Path, Args, Opts) ->
    ...
    case websocket_handshake(WSReq, ExtraHeaders) of
        ...
        {ok, Buffer} ->
            ...
            websocket_loop(ws_req:set([{keepalive,KeepAlive},{keepalive_timer,KATimer}], WSReq), HandlerState, <<>>)
  end.

%% @doc Send http upgrade request and validate handshake response challenge
-spec websocket_handshake(WSReq :: ws_req:req(), [{string(), string()}]) -> {ok, binary()} | {error, term()}.
websocket_handshake(WSReq, ExtraHeaders) ->
    [Path, Host, Key, Transport, Socket] =
        ws_req:get([path, host, key, transport, socket], WSReq),
...
-spec send(ws_req:frame(), ws_req:req()) -> ok | {error, term()}.
send(Frame, WSReq) ->
  Socket = ws_req:socket(WSReq),
  Transport = ws_req:transport(WSReq),
  Transport:send(Socket, encode_frame(Frame)).

%% @doc Main loop
-spec websocket_loop(WSReq :: ws_req:req(), HandlerState :: any(),
                     Buffer :: binary()) ->
                            ok.
...
handle_websocket_message(WSReq, HandlerState, Buffer, Message) ->
    [Handler, Remaining, Socket] =
        ws_req:get([handler, remaining, socket], WSReq),
    case Message of
        keepalive ->
            cancel_keepalive_timer(WSReq),
            % ok = send({ping, <<>>}, WSReq),
            send({binary, maps:get(heart, HandlerState, <<>>)}, WSReq),
            KATimer = erlang:send_after(ws_req:keepalive(WSReq), self(), keepalive),
            websocket_loop(ws_req:keepalive_timer(KATimer, WSReq), HandlerState, Buffer);
        {cast, Frame} ->
            ok = send(Frame, WSReq),
            websocket_loop(WSReq, HandlerState, Buffer);
        {_Closed, Socket} ->
        ...
        {_TransportType, Socket, Data} ->
            case Remaining of
                ...
                _ ->
                    retrieve_frame(WSReq, HandlerState,
                                   ws_req:opcode(WSReq), Remaining, Data, Buffer)
            end;
        Msg ->
...
-spec cancel_keepalive_timer(ws_req:req()) -> ok.
cancel_keepalive_timer(WSReq) ->
    case ws_req:keepalive_timer(WSReq) of
        undefined ->
            ok;
        OldTimer ->
...
-spec websocket_close(WSReq :: ws_req:req(),
                      HandlerState :: any(),
                      Reason :: tuple()) -> ok.
websocket_close(WSReq, HandlerState, Reason) ->
    Handler = ws_req:handler(WSReq),
...
retrieve_frame(WSReq, HandlerWSReq, Opcode, Len, Data, Buffer)
  when byte_size(Data) < Len ->
    ...
    WSReq1 = ws_req:remaining(Remaining, WSReq),
    WSReq2  = ws_req:opcode(Opcode, WSReq1),
...
retrieve_frame(WSReq, HandlerState, Opcode, Len, Data, Buffer) ->
    [Handler, Continuation, ContinuationOpcode] =
        ws_req:get([handler, continuation, continuation_opcode], WSReq),
    Fin = ws_req:fin(WSReq),
    << Payload:Len/binary, Rest/bits >> = Data,
    FullPayload = << Buffer/binary, Payload/binary >>,
    OpcodeName = ws_req:opcode_to_name(Opcode),
    case OpcodeName of
        ping ->
    ...
    case OpcodeName of
        ...
        _ when Opcode < 8, Continuation =/= undefined, Fin == 0 ->
            ...
            WSReq1 = ws_req:continuation(Continuation1, WSReq),
        ...
        _ when Opcode < 8, Continuation =/= undefined, Fin == 1 ->
            ...
            WSReq1 = ws_req:continuation(undefined, WSReq),
            WSReq2 = ws_req:continuation_opcode(undefined, WSReq1),
            ContinuationOpcodeName = ws_req:opcode_to_name(ContinuationOpcode),
            try Handler:websocket_handle(
                                {ContinuationOpcodeName, DefragPayload},
                                WSReq2, HandlerState) of
                HandlerResponse ->
                    handle_response(ws_req:remaining(undefined, WSReq1),
        ...
        _ ->
            try Handler:websocket_handle(
                ...
                HandlerResponse ->
                    handle_response(ws_req:remaining(undefined, WSReq),
                                    HandlerResponse, Rest)
            catch _:Reason:_ ->
                websocket_close(WSReq, HandlerState, {handler, Reason})
            end
...
%% masks the data
-spec encode_frame(ws_req:frame()) ->
                          binary().
encode_frame({Type, Payload}) ->
    Opcode = ws_req:name_to_opcode(Type),
...
-spec set_continuation_if_empty(WSReq :: ws_req:req(),
                                Opcode :: ws_req:opcode()) ->
                                       ws_req:req().
set_continuation_if_empty(WSReq, Opcode) ->
    case ws_req:continuation(WSReq) of
        undefined ->
            WSReq1 = ws_req:continuation_opcode(Opcode, WSReq),
            ws_req:continuation(<<>>, WSReq1);
        _ ->
            WSReq
    end.
